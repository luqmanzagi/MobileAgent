...
from PCAgent.api_gemini import inference_chat
...
ZERO_WIDTH = ''.join(chr(c) for c in [
    0x200B, 0x200C, 0x200D, 0xFEFF, 0x2060, 0x180E
])
...
def safe_json_from_text(text: str) -> dict:
    if not isinstance(text, str):
        raise ValueError("Expected string model output")

    # Remove zero-width / BOM-ish characters
    text = text.translate({ord(c): None for c in ZERO_WIDTH})

    candidate = _first_fenced_json(text)
    if candidate is None:
        candidate = _first_balanced_brace_block(text)
    if candidate is None:
        raise ValueError("No JSON object found in model output")

    candidate = _normalize_quotes(candidate)
    candidate = _remove_json_comments(candidate)
    candidate = _strip_trailing_commas(candidate).strip()

    try:
        return json.loads(candidate)
    except json.JSONDecodeError as e:
        # Last-ditch: sometimes keys/strings come single-quoted; try converting to double-quoted safely
        c2 = re.sub(r"(?<!\\)'", '"', candidate)  # naive but often good enough
        c2 = _strip_trailing_commas(_remove_json_comments(c2))
...
def record_usage(model_name: str, usage: dict):
    """Route usage into the LLM or VL bucket based on which model was called."""
    kind = "llm" if model_name == llm_model_version else "vl"
    TOKEN_BANK[kind]["prompt"]     += int(usage.get("prompt", 0) or 0)
    TOKEN_BANK[kind]["completion"] += int(usage.get("completion", 0) or 0)

def cost_for(model_name: str, prompt_tokens: int, completion_tokens: int) -> float:
    rates = PRICING.get(model_name, {})
    rp = float(rates.get("prompt", 0.0))
    rc = float(rates.get("completion", 0.0))
    # prices in pricing.json are treated as USD per token
    return prompt_tokens * (rp/1000.0) + completion_tokens * (rc/1000.0)
...
PRICING = json.load(open('pricing.json', 'r'))  # {"model": {"prompt": rate_per_token, "completion": rate_per_token}}
TOKEN_BANK = {
    "llm": {"prompt": 0, "completion": 0},
    "vl":  {"prompt": 0, "completion": 0},
}

token_data = json.load(open('config_gemini.json', 'r'))
...
def select(content, screenshot_file):
    ...
    output_select, usage = inference_chat(chat_select, vl_model_version, API_url, token)
    record_usage(vl_model_version, usage)   
...
if args.simple == 1:
    ...
    for i in range(num_try_subtask):
        try:
            ...
            output_subtask, usage = inference_chat(chat_subtask, llm_model_version, API_url, token)
            record_usage(llm_model_version, usage)
...
for i in range(num_subtask):
    ...
    while True:
        ...
        output_action, usage = inference_chat(chat_action, vl_model_version, API_url, token)
        record_usage(vl_model_version, usage)
        ...
        action_json = safe_json_from_text(output_action)
        ...
        if memory_switch:
            ...
            output_memory, usage = inference_chat(chat_action, vl_model_version, API_url, token)
            record_usage(vl_model_version, usage)
        ...
        if reflection_switch:
            ...
            output_reflect, usage = inference_chat(chat_reflect, vl_model_version, API_url, token)
            record_usage(vl_model_version, usage)
            ...
            if True:
                ...
                output_planning, usage = inference_chat(chat_planning, llm_model_version, API_url, token)
                record_usage(llm_model_version, usage)
